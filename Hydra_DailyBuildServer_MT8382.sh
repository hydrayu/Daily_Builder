...
if [ ! -d $SRC/Daily_build/MT8382 ]; then
	echo "Building Daily Build MT8382 Directory..."
...
if [ "$ORGCMNUM" != "$NCMNUM"  ] ||  [ "$NCMDATE" != "$ORGCMDATE"  ];then
	...
	echo " Copying Image..."
	mkdir "$DB"/MT8382_"$CDATE"_"$BRANCH"_"$NCMNUM" 
	echo yes | ~/bin/cp_MTK_pandora_img out/target/product/mt82kk_tb "$DB"/MT8382_"$CDATE"_"$BRANCH"_"$NCMNUM"
	echo "Done !!!"
else 
